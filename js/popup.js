...
$(function () {
    ...
    // 优化的关键词更新处理
    const handleKeywordInput = Utils.debounce(async function(element, categoryIndex) {
        try {
    ...
    $textBox.on('input', '.content', function() {
        const index = Array.from(document.querySelectorAll('.content')).indexOf(this);
    ...
    function renderListItems(items) {
        ...
        items.forEach(data => {
            ...
            // 设置颜色
            $item.find('.colorSelect')
                .removeClass((_, className) => (className.match(/(^|\s)chrome-extension-mutihighlight-style-\S+/g) || []).join(' '))
    ...
    $textBox.on('click', '.setBtn', async function (e) {
        ...
        try {
            ...
            $btn.find('.el-switch__core').css({
                'background-color': currentStatus ? 'var(--el-color-primary)' : '#dcdfe6',
        ...
        try {
            ...
            $target
                .removeClass(function (index, className) {
                    return (className.match(/(^|\s)chrome-extension-mutihighlight-style-\d+/g) || []).join(' ');
                })
    ...
    $("#add").on("click", async function() {
        try {
            ...
            // 设置初始状态
            $newItem.find('.colorSelect').addClass('chrome-extension-mutihighlight-style-1');
            $newItem.find('.setBtn').attr("data-on", "true").find('.el-switch__core').css({
                'background-color': 'var(--el-color-primary)',
                'border-color': 'var(--el-color-primary)'
            });
    ...
    $("#dedupe").click(async function () {
        ...
        const newListData = listData.map((item, categoryIndex) => {
            ...
            const uniqueWords = Array.from(new Set(words)).filter(word => {
                ...
                return firstOccurrence && firstOccurrence.categoryIndex === categoryIndex;
            });
        ...
        try {
            ...
            changedCategories.forEach(index => {
                $(`.content`).eq(index).val(newListData[index].data || '');
            });
    ...
    $("#fileInput").change(function (e) {
        ...
        reader.onload = async function (e) {
            try {
                ...
                showCustomConfirm('确定要导入配置吗？这将覆盖当前的所有配置。', async () => {
                    try {
                        // 清空当前列表
                        $textBox.empty();
    ...
    $("#applyCode").click(async function () {
        ...
        try {
            ...
            showCustomConfirm('确定要应用这个配置吗？这将与当前配置合并。', async () => {
                try {
                    // 合并配置
                    const newData = [...listData];
                    importedData.forEach(item => {
                        // 检查否已存在相同名称置
                        const existingIndex = newData.findIndex(existing =>
    ...
    function initDragAndDrop() {
        ...
        container.find('.highlight-item').each((index, item) => {
            ...
            $item.on('mousedown', (e) => {
                // 排除所有交互元素
                if ($(e.target).is('input, textarea, button, .colorSelect, .setBtn, .rem') || 
                ...
                $(document).on('mousemove.drag', (e) => {
                    ...
                    if (targetIndex !== -1) {
                        ...
                        const isAfter = e.clientY > $target[0].getBoundingClientRect().top + $target.height() / 2;
                    ...
                    if (e.clientY < containerTop + scrollMargin) {
                        const speed = Math.ceil((scrollMargin - (e.clientY - containerTop)) / scrollMargin * maxSpeed);
                        container.scrollTop(container.scrollTop() - speed);
                    } else if (e.clientY > containerBottom - scrollMargin) {
                        const speed = Math.ceil((scrollMargin - (containerBottom - e.clientY)) / scrollMargin * maxSpeed);
                        container.scrollTop(container.scrollTop() + speed);
                    }
                });
            ...
            <div class="colorBox" style="display:none;">
                <div class="color-picker__grid">
                    ${Array.from({ length: 20 }, (_, i) => `
                        <div class="color-picker__item chrome-extension-mutihighlight-style-${i + 1}" 
    ...
    async function checkUpdate() {
        try {
            ...
            // 添加时间戳防止缓存
            const response = await fetch('https://gengxin.geluman.cn/version.json?' + Date.now(), {
                method: 'GET',
                headers: {
    ...
    function showUpdateDialog(updateData) {
        ...
        // 下载按钮事件
        $('#downloadUpdate').off('click').on('click', function () {
